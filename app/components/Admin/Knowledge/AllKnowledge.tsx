...
import { useDeleteKnowledgeMutation, useGetAllKnowlegeQuery } from "@/redux/features/knowledge/knowledgeApi";
...
const AllKnowledge = (props: Props) => {
    const { theme, setTheme } = useTheme();
    const [open, setOpen] = useState(false);
    const [openModalAdd, setOpenModalAdd] = useState(false);
    const [blogId, setBlogId] = useState("");
    const { isLoading, data, refetch } = useGetAllKnowlegeQuery(
        {},
        { refetchOnMountOrArgChange: true }
    );
    const [deleteBlog, { isSuccess, error, isLoading: isLoadingDel }] = useDeleteKnowledgeMutation({});
    ...
    const columns = [
        ...
        { field: "created_at", headerName: "Created At", flex: 0.5 },
        {
            ...
            renderCell: (params: any) => {
                return (
                    <>
                        <Link href={`/admin/edit-knowledge/${params.row.id}`}>
    ...
    const selectBlog = data?.result?.find(ele => ele._id === blogId)
            ...
            {isLoading ? (
                ...
                <Box m="20px">
                    <div className="w-full flex justify-end">
                        <Link href={'/admin/create-knowledge'} className="flex items-center justify-center  h-[40px] w-[150px]  ml-4 font-size-[12px]  md:font-size-[16px] text-[16px] font-medium leading-[56px] bg-[#FFFFFF] text-[#000000]  border-solid border-2 border-[#ffcf66] box-shadow-grow hover:text-black cursor-pointer hover:border-[#ffcf66]  rounded-lg text-center px-[20px] md:px-[40px]">
                            + Add
                        </Link>
                    </div>
                    <Box
